...
//acesso dos vetores com os respectivos parametros para a execução
...
//set parametros para execução e para retorno
...
void ProcTask::setParms(vector<string> param_of_cham_,vector<string> param_of_return_,int ID_by_executed_,int ref_of_return_,lua_State* store_val_){
	...
	//tilizamos os valores setados pois não é necessário acessar a bag para este
	//use_Params_Set = true;

	this->paramOfCham = param_of_cham_;
	this->paramOfReturn = param_of_return_;

	//local onde o retorno da funçao eecutada em paralelo dev ser dado
...
void ProcTask::run() {
	...
	while(!Bag->empty()){
		...
		if (tempParam.getSetParam().size() > 0) {
			...
			//fechamento ocorre só aqui (uma vez, se a pilha não recebe parametros ela não é declarada)
...
//qWarning(buffer);
		...
		for(int ind = 0;ind < tempParam.getSetParam().size();ind++){
			...
			#else
				//para linux(não existe itoa em linux)
		...
		//qualquer estado diferebte exige desalocação e temos que construir uma nova co-routine
