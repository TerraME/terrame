...
Agent_ = {
	...
	--- Put the Agent into a Cell, using the placements of both. Before using enter() it is necessary to create a placement for the Agent.
	-- @param cell A Cell.
  ...
  --end}
	execute = function(self, event)
		if(type(event) == "Event" or type(event) == "userdata") then
	...
	getCells = function(agent, placement)
		if type(placement) ~= "string" then
			if placement == nil  then
				placement = "placement"
				defaultValueWarningMsg("#1", placement, 3)
			else
				incompatibleTypesErrorMsg("#1", "string", type(placement), 3)
			end
		end
	...
	--- A user-defined function that is used to initialize an Agent when it enters in a given Society.
	-- @see Society:add
	-- @usage agent:init()
	...
	--- Send a message to another Agent as a table. They can arrive exactly after they are sent (synchronous) or have some delay (asynchronous). In the later case, it is necessary to call function synchronize from the Society they belong to activate such messages.
	-- @param data.receiver The Agent that will get the message.
	-- @param data.subject A string describing the function that will be called in the receiver. Given a string x, the receiver will get the message in a function called on_x. Default is "message". The function to receive the message must be implemented by the modeler. See Agent::on_* for more details.
	...
	--- Move the Agent to a new Cell. Before using move() it is necessary to create a placement for the Agent.
	-- @param newcell The new Cell.
	-- @param placement A string representing the index to be used. Default is "placement".
	-- @usage agent:move(cell, "renting")
	-- @see Environment:createPlacement
	...
	-- @see Agent:message
	-- @see Society:synchronize
	-- @usage agent = Agent{on_message = function(self, message)
	--     self.money = self.money + message.quantity
	--     self:message{receiver = message.sender, content = "thanks"}
	-- end}
	--
	-- agent = Agent{on_thanks = function(self, message)
	...
	--
	-- agent:randomWalk("moore")
	randomWalk = function(self, placement, neighborhood, randomObj)
		if type(placement) ~= "string" then
			if placement == nil then
				placement = "placement"
				defaultValueWarningMsg("#1", "string", placement, 3) 
	...
	reproduce = function(self, data)
		...
		if type(data) ~= "table" then
		    if data == nil then
		        data = {}
		        incompatibleTypesWarningMsg("#1", "table","nil", 3)
		    else
		        incompatibleTypesErrorMsg("#1", "table", type(data), 3) 
	        end
		end
...
-- such as init(), on_message(), and execute().
...
-- @output cells A vector of Cells necessary to use forEachCell(agent). This value is the same of "agent.placement.cells".
...
-- @output socialnetworks A set of SocialNetwork, with the connections of the Agent.
-- @output type A string containing "Agent".
-- @see Society
-- @see SocialNetwork
-- @see Environment:createPlacement
