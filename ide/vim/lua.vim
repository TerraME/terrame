...
" Comments
syn keyword luaTodo            contained TODO FIXME XXX SKIP DONTRUN
syn region luaDoc              contained start=" @arg \| @" end=" "
syn match   luaComment         "--.*$" contains=luaTodo,luaDoc,@Spell
...
elseif lua_version == 5
  ...
  if lua_subversion == 0
    ...
    syn keyword luaFunc assert assertError assertNil assertEquals assertType createNeighborhood createPlacement createSocialNetwork getConfig load select switch forEachAgent forEachCell forEachCellPair forEachElement forEachOrderedElement forEachNeighbor forEachConnection forEachNeighborhood Agent Map Chart Clock Cell CellularSpace Event Group Society Timer Trajectory Environment Project Layer
